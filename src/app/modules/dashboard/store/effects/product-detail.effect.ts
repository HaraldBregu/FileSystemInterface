...
import { createVariantSuccess, deleteVariantSuccess } from "../actions/variant.actions";
...
export class ProductDetailEffects {
    ...
    /// GET PRODUCT DETAILS 
    /// ON CREATED VARIANT SUCCESS
    getProductDetailOnCreatedVariantSuccess = createEffect(() => this.actions$.pipe(
        ofType(createVariantSuccess),
        withLatestFrom(
            this.store.pipe(select(selectedCatalogName)),
            this.store.pipe(select(selectedProductId))
        ),
    ...
    /// GET PRODUCT DETAILS 
    /// ON DELETE VARIANT SUCCESS
    getProductDetailOnDeleteVariantSuccess = createEffect(() => this.actions$.pipe(
        ofType(deleteVariantSuccess),
        withLatestFrom(
            this.store.pipe(select(selectedCatalogName)),
            this.store.pipe(select(selectedProductId))
        ),
